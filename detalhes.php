...
$sql = "SELECT r.*, c.nome AS categoria FROM restaurantes r LEFT JOIN categorias c ON r.categoria_id = c.id WHERE r.id = ?";
...
$sql = "SELECT a.comentario, a.nota, a.data_avaliacao, u.nome FROM avaliacoes a JOIN utilizadores u ON a.user_id = u.id WHERE a.restaurante_id = ? ORDER BY a.data_avaliacao DESC";
$stmt = $pdo->prepare($sql);
$stmt->execute([$id]);
$avaliacoes = $stmt->fetchAll(PDO::FETCH_ASSOC);
// After fetching $restaurante
$is_favorito = false;
if (isset($_SESSION['user_id'])) {
    $sql = "SELECT id FROM favoritos WHERE user_id = ? AND restaurante_id = ?";
    $stmt = $pdo->prepare($sql);
...
<?php if (isset($_SESSION['user_id'])): ?>
    <form action="favoritos.php" method="post">
        ...
        <button type="submit" class="btn"><?= $is_favorito ? 'Remover dos Favoritos' : 'Adicionar aos Favoritos' ?></button>
    </form>
<?php endif; ?>
?>
<!DOCTYPE html>
...
<body>
    ...
    <main>
        <section>
            <img src="get_imagem.php?id=<?= $restaurante['id'] ?>" alt="<?= htmlspecialchars($restaurante['nome']) ?>" style="max-width: 300px;">
        ...
        <section>
            ...
            <?php if ($avaliacoes): ?>
                <?php foreach ($avaliacoes as $avaliacao): ?>
                    <div class="avaliacao">
                        <p><strong><?= htmlspecialchars($avaliacao['nome']) ?></strong> (Nota: <?= $avaliacao['nota'] ?>/5)</p>
